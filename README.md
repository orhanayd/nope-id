...
- **Faster** - 2x to 9x faster than nanoid (CSPRNG, full URL-safe alphabet); wins all 5 core benchmarks ([see benchmarks](#performance))
<!-- bench:headline:end -->
...
<!-- bench:meta:start -->
_Last refreshed: 2026-05-28, Node v26.x, ubuntu-latest (GitHub Actions)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Basic (21 chars) | ~5.5M ops/sec | **~39.5M ops/sec** | **nope-id ~7.2x** |
| Small (10 chars) | ~9.9M ops/sec | **~44.2M ops/sec** | **nope-id ~4.5x** |
| Large (64 chars) | ~2.0M ops/sec | **~18.8M ops/sec** | **nope-id ~9.3x** |
| Custom Alphabet | ~5.7M ops/sec | **~12.4M ops/sec** | **nope-id ~2.2x** |
| Batch (100 IDs) | ~56K ops/sec | **~435K ops/sec** | **nope-id ~7.8x** |
...
| Generator | ops/sec | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~21.2M | C++ binding (plain v4 only) |
| nope-id `uuid()` (v4) | **~25M** | 🥇 fastest pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~6.9M | optimized pure-JS v4 |
| `uuid` package `v4()` | ~5.9M | |
| nope-id `uuidv7()` | ~5.3M | **~12x the `uuid` package's v7** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~39.5M** | **~126 bits (64-char URL-safe)** | **CSPRNG** |
| `uid/secure` | ~6.3M | ~84 bits (16-char hex) | CSPRNG |
| nanoid | ~5.5M | ~126 bits (64-char URL-safe) | CSPRNG |
| `sparkid` | ~9.8M | ~76 bits random (Base58, time-sortable) | CSPRNG |
| `rndm` | ~2.8M | ~125 bits, but predictable | `Math.random` (not secure) |
| `secure-random-string` | ~394K | ~126 bits (base64, not URL-safe) | CSPRNG |
...
- **nanoid** matches nope-id's entropy exactly (same 64-char alphabet); nope-id is simply <!-- bench:basic-21-ratio:start -->~7.2x<!-- bench:basic-21-ratio:end --> faster at the default 21-char size.
...
| `prefixedId()` | ~28.4M ops/sec |
| `uuid()` | ~24.9M ops/sec |
| `slugId()` | ~5.6M ops/sec |
| `shortId()` | ~11.3M ops/sec |
| `isValid()` | ~6.7M ops/sec |
| `uuidv7()` | ~5.3M ops/sec |
| `ulid()` | ~2.8M ops/sec |
| `monotonicFactory()` | ~8.5M ops/sec |
| `snowflake` (factory) | ~4.1M ops/sec |
| `objectId()` | ~7.2M ops/sec |
| `sqids.encode()` | ~210K ops/sec |
