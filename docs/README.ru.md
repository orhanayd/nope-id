...
|------|--------|---------|--------|
| Базовый (21 символ) | ~26.5M оп/сек | **~57.9M оп/сек** | **nope-id ~2.2x** |
| Короткий (10 символов) | ~35.5M оп/сек | **~52.3M оп/сек** | **nope-id ~1.5x** |
| Длинный (64 символа) | ~10.6M оп/сек | **~34M оп/сек** | **nope-id ~3.2x** |
| Свой алфавит | ~32.1M оп/сек | **~66.2M оп/сек** | **nope-id ~2.1x** |
| Пакетный (100 ID) | ~246K оп/сек | **~702K оп/сек** | **nope-id ~2.9x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~21.6M | C++ binding (только обычный v4) |
| nope-id `uuid()` (v4) | **~30.4M** | 🥇 самый быстрый pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~6.7M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~5.8M | |
| nope-id `uuidv7()` | ~9.4M | **~20x от v7 пакета `uuid`** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~57.9M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~6.2M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~26.5M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
...
| `secure-random-string` | ~426K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
...
|---------|-------------|
| `sortableId()` | ~14.2M оп/сек |
| `prefixedId()` | ~36.4M оп/сек |
| `uuid()` | ~31.1M оп/сек |
| `slugId()` | ~7.2M оп/сек |
| `shortId()` | ~15.8M оп/сек |
| `isValid()` | ~13.9M оп/сек |
| `uuidv7()` | ~9.0M оп/сек |
| `ulid()` | ~13.8M оп/сек |
| `monotonicFactory()` | ~8.4M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~15.4M оп/сек |
