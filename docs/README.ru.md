...
<!-- bench:meta:start -->
_Последнее обновление: 2026-07-14, Node v22.x, darwin/arm64 (локально)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~37.5M оп/сек | **~85.8M оп/сек** | **nope-id ~2.3x** |
| Короткий (10 символов) | ~50.9M оп/сек | **~79.6M оп/сек** | **nope-id ~1.6x** |
| Длинный (64 символа) | ~14.7M оп/сек | **~51.1M оп/сек** | **nope-id ~3.5x** |
| Свой алфавит | ~44.6M оп/сек | **~82.5M оп/сек** | **nope-id ~1.9x** |
| Пакетный (100 ID) | ~367K оп/сек | **~1.1M оп/сек** | **nope-id ~3.1x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | **~27.7M** | 🥇 самый быстрый для обычного v4 |
| nope-id `uuid()` (v4) | ~26.3M | ~3.3x быстрее `@lukeed/uuid`, быстрее пакета `uuid` |
| `@lukeed/uuid` `v4()` | ~8.0M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~8.1M | |
| nope-id `uuidv7()` | ~13.5M | **~13x от v7 пакета `uuid`** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~85.8M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~9.7M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~37.5M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
| `sparkid` | ~16.2M | ~76 бит случайных (Base58, сортируемый по времени) | CSPRNG |
| `rndm` | ~3.6M | ~125 бит, но предсказуемые | `Math.random` (не безопасный) |
| `secure-random-string` | ~806K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
| cuid2 `createId()` | ~7.7K | 24 символа, hash-производный | CSPRNG + SHA-3 |
...
|---------|-------------|
| `sortableId()` | ~22.5M оп/сек |
| `prefixedId()` | ~55.1M оп/сек |
| `uuid()` | ~26.1M оп/сек |
| `slugId()` | ~10M оп/сек |
| `shortId()` | ~23.5M оп/сек |
| `isValid()` | ~20.8M оп/сек |
| `uuidv7()` | ~13.1M оп/сек |
| `ulid()` | ~18.4M оп/сек |
| `monotonicFactory()` | ~14.8M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~19.8M оп/сек |
| `sqids.encode()` | ~330K оп/сек |
