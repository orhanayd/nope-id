...
<!-- bench:meta:start -->
_Последнее обновление: 2026-09-22, Node v26.x, ubuntu-latest (GitHub Actions)._
<!-- bench:meta:end -->
...
| Тест | nanoid 6.0.1 | nope-id | Победитель |
|------|--------|---------|--------|
| Базовый (21 символ) | ~26.7M оп/сек | **~55.7M оп/сек** | **nope-id ~2.1x** |
| Короткий (10 символов) | ~35.3M оп/сек | **~51.5M оп/сек** | **nope-id ~1.5x** |
| Длинный (64 символа) | ~10.8M оп/сек | **~32.9M оп/сек** | **nope-id ~3x** |
| Свой алфавит | ~32.4M оп/сек | **~67.2M оп/сек** | **nope-id ~2.1x** |
| Пакетный (100 ID) | ~258K оп/сек | **~759K оп/сек** | **nope-id ~2.9x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~19.4M | C++ binding (только обычный v4) |
| nope-id `uuid()` (v4) | **~31.6M** | 🥇 самый быстрый pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~6.8M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~5.9M | |
| nope-id `uuidv7()` | ~12M | **~31x от v7 пакета `uuid`** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~55.7M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~6.4M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~26.7M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
...
| `secure-random-string` | ~345K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
...
|---------|-------------|
| `sortableId()` | ~12.9M оп/сек |
| `prefixedId()` | ~36.7M оп/сек |
| `uuid()` | ~31.9M оп/сек |
| `slugId()` | ~15.6M оп/сек |
| `shortId()` | ~20.7M оп/сек |
| `isValid()` | ~15.5M оп/сек |
| `uuidv7()` | ~12M оп/сек |
| `ulid()` | ~11.6M оп/сек |
| `monotonicFactory()` | ~14M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~13.4M оп/сек |
| `sqids.encode()` | ~876K оп/сек |
