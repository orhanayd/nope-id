...
|------|--------|---------|--------|
| Базовый (21 символ) | ~34.5M оп/сек | **~74.6M оп/сек** | **nope-id ~2.2x** |
| Короткий (10 символов) | ~44.8M оп/сек | **~66.5M оп/сек** | **nope-id ~1.5x** |
| Длинный (64 символа) | ~13.8M оп/сек | **~43.7M оп/сек** | **nope-id ~3.2x** |
| Свой алфавит | ~41.2M оп/сек | **~83.3M оп/сек** | **nope-id ~2x** |
| Пакетный (100 ID) | ~319K оп/сек | **~982K оп/сек** | **nope-id ~3.1x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~25.1M | C++ binding (только обычный v4) |
| nope-id `uuid()` (v4) | **~37.7M** | 🥇 самый быстрый pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~8.3M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~7.5M | |
| nope-id `uuidv7()` | ~10.7M | **~21x от v7 пакета `uuid`** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~74.6M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~8.0M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~34.5M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
| `sparkid` | ~12M | ~76 бит случайных (Base58, сортируемый по времени) | CSPRNG |
| `rndm` | ~3.5M | ~125 бит, но предсказуемые | `Math.random` (не безопасный) |
| `secure-random-string` | ~448K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
| cuid2 `createId()` | ~6.6K | 24 символа, hash-производный | CSPRNG + SHA-3 |
...
|---------|-------------|
| `sortableId()` | ~16.7M оп/сек |
| `prefixedId()` | ~47.4M оп/сек |
| `uuid()` | ~38.8M оп/сек |
| `slugId()` | ~9.4M оп/сек |
| `shortId()` | ~20.8M оп/сек |
| `isValid()` | ~17.8M оп/сек |
| `uuidv7()` | ~10.7M оп/сек |
| `ulid()` | ~14.9M оп/сек |
| `monotonicFactory()` | ~9.6M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~16.3M оп/сек |
| `sqids.encode()` | ~277K оп/сек |
