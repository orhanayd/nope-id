...
- **Быстрее** - в 1.5–4 раз быстрее nanoid (CSPRNG, полный URL-безопасный алфавит); выигрывает все 5 основных бенчмарков ([см. бенчмарки](#производительность))
<!-- bench:headline:end -->
...
- **Хорошо протестирован** - 408 тестов, включая тесты безопасности и энтропии ([тестирование](#тестирование))
...
nope-id имеет покрытие из **408 тестов** по 11 наборам, включая security-специфичные тесты.
...
| **Alphabet Tiers** | 21 | тиры пополнения customAlphabet, чанкование customRandom |
| **Pack Manifest** | 2 | точный список файлов npm-тарбола (посторонние файлы не пройдут) |
| **Всего** | **408** | Все проходят |
...
<!-- bench:meta:start -->
_Последнее обновление: 2026-07-14, Node v22.x, linux/x64 (локально)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~25.6M оп/сек | **~57.7M оп/сек** | **nope-id ~2.3x** |
| Короткий (10 символов) | ~34.8M оп/сек | **~54.7M оп/сек** | **nope-id ~1.6x** |
| Длинный (64 символа) | ~9.8M оп/сек | **~36.3M оп/сек** | **nope-id ~3.7x** |
| Свой алфавит | ~29M оп/сек | **~53.2M оп/сек** | **nope-id ~1.8x** |
| Пакетный (100 ID) | ~212K оп/сек | **~747K оп/сек** | **nope-id ~3.5x** |
...
- **Пулированный CSPRNG:** одно заполнение `randomFillSync()` (в браузере `crypto.getRandomValues()`) покрывает тысячи ID при дефолтном размере вместо одного syscall на ID. `uuid()` идёт дальше: предварительно форматирует 1820 v4 UUID (с уже патченными битами версии + RFC 4122 variant) в одну строку из 65520 символов — размер выбран так, чтобы оставаться ниже порога аллокации больших строк в V8 — так что каждый вызов - это просто `substring(start, start+36)`. `uuidv7()` делает то же для случайной части: пул из 2048 предварительно отформатированных 21-символьных хвостов с уже встроенными дефисами и variant `[89ab]`; каждый вызов - это кешированный префикс мс плюс один `substring`.
...
- **`customAlphabet` с тирами по форме алфавита:** фабрика выбирает стратегию пополнения один раз на алфавит: полностью нативное hex-кодирование для hex-алфавитов, branch-free bulk-перевод для длин-степеней-двойки и 16-битный двухзначный rejection sampling для всего остального — каждый принятый uint16 ниже наибольшего кратного len² даёт **два** равномерных символа, что примерно удваивает символы на байт CSPRNG и вдвое сокращает итерации цикла приёма (немногие длины алфавита, где побайтовая выборка всё ещё выгоднее, 182-255, остаются на ней). Hot path всегда один `substring(start, end)`; это также ускоряет `slugId()` и `shortId()`.
- **Кеш временного префикса:** `sortableId()`, `ulid()`, `uuidv7()` и `objectId()` перекодируют свой префикс метки времени только когда значение часов действительно меняется, а случайные хвосты берут из заранее закодированных пул-строк — `ulid()` перепаковывает каждое 32-битное слово CSPRNG в шесть несмещённых 5-битных цифр Crockford — так что путь на вызов - это короткая конкатенация строк. `objectId()` и `monotonicFactory()` дополнительно кешируют всё, кроме последнего горячего символа, так что обычный случай - конкатенация из двух частей.
- **Precomputed таблицы для всего остального:** упакованные 16-битные byte-to-hex записи для `uuid()`/`uuidv7()`, char-коды Crockford для `ulid()`/`monotonicFactory()`, плюс переиспользуемые scratch-буферы уровня модуля, так что путь на каждый вызов никогда не аллоцирует.
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~57.7M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~5.8M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~25.6M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
| `sparkid` | ~10.3M | ~76 бит случайных (Base58, сортируемый по времени) | CSPRNG |
| `rndm` | ~2.7M | ~125 бит, но предсказуемые | `Math.random` (не безопасный) |
| `secure-random-string` | ~407K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
| cuid2 `createId()` | ~5.3K | 24 символа, hash-производный | CSPRNG + SHA-3 |
...
- **nanoid** точно соответствует энтропии nope-id (тот же 64-символьный алфавит); nope-id просто в <!-- bench:basic-21-ratio:start -->~2.3x<!-- bench:basic-21-ratio:end --> раза быстрее на дефолтном 21-символьном размере.
...
|---------|-------------|
| `sortableId()` | ~13.8M оп/сек |
| `prefixedId()` | ~37.1M оп/сек |
| `uuid()` | ~24.3M оп/сек |
| `slugId()` | ~17.3M оп/сек |
| `shortId()` | ~22.2M оп/сек |
| `isValid()` | ~17.6M оп/сек |
| `uuidv7()` | ~12.1M оп/сек |
| `ulid()` | ~12.9M оп/сек |
| `monotonicFactory()` | ~16.4M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~15.6M оп/сек |
| `sqids.encode()` | ~805K оп/сек |
