...
<!-- bench:meta:start -->
_Последнее обновление: 2026-07-14, Node v26.x, ubuntu-latest (GitHub Actions)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~36.1M оп/сек | **~78.2M оп/сек** | **nope-id ~2.2x** |
| Короткий (10 символов) | ~48.7M оп/сек | **~73.8M оп/сек** | **nope-id ~1.5x** |
| Длинный (64 символа) | ~13.8M оп/сек | **~46.6M оп/сек** | **nope-id ~3.4x** |
| Свой алфавит | ~41M оп/сек | **~91.8M оп/сек** | **nope-id ~2.2x** |
| Пакетный (100 ID) | ~343K оп/сек | **~1.0M оп/сек** | **nope-id ~3x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~27.3M | C++ binding (только обычный v4) |
| nope-id `uuid()` (v4) | **~42.7M** | 🥇 самый быстрый pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~9.0M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~7.2M | |
| nope-id `uuidv7()` | ~16.8M | **~26x от v7 пакета `uuid`** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~78.2M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~7.6M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~36.1M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
| `sparkid` | ~12.6M | ~76 бит случайных (Base58, сортируемый по времени) | CSPRNG |
| `rndm` | ~3.2M | ~125 бит, но предсказуемые | `Math.random` (не безопасный) |
| `secure-random-string` | ~503K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
| cuid2 `createId()` | ~6.4K | 24 символа, hash-производный | CSPRNG + SHA-3 |
...
- **nanoid** точно соответствует энтропии nope-id (тот же 64-символьный алфавит); nope-id просто в <!-- bench:basic-21-ratio:start -->~2.2x<!-- bench:basic-21-ratio:end --> раза быстрее на дефолтном 21-символьном размере.
...
|---------|-------------|
| `sortableId()` | ~16.6M оп/сек |
| `prefixedId()` | ~49.6M оп/сек |
| `uuid()` | ~41.4M оп/сек |
| `slugId()` | ~20.4M оп/сек |
| `shortId()` | ~27.6M оп/сек |
| `isValid()` | ~16M оп/сек |
| `uuidv7()` | ~16.7M оп/сек |
| `ulid()` | ~15.6M оп/сек |
| `monotonicFactory()` | ~19.3M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~18.3M оп/сек |
| `sqids.encode()` | ~989K оп/сек |
