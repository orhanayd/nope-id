...
<!-- bench:meta:start -->
_Последнее обновление: 2026-07-16, Node v26.x, ubuntu-latest (GitHub Actions)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~27.4M оп/сек | **~58.2M оп/сек** | **nope-id ~2.1x** |
| Короткий (10 символов) | ~36.9M оп/сек | **~52.2M оп/сек** | **nope-id ~1.4x** |
| Длинный (64 символа) | ~10.8M оп/сек | **~33.8M оп/сек** | **nope-id ~3.1x** |
| Свой алфавит | ~32.8M оп/сек | **~70M оп/сек** | **nope-id ~2.1x** |
| Пакетный (100 ID) | ~269K оп/сек | **~779K оп/сек** | **nope-id ~2.9x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~20.7M | C++ binding (только обычный v4) |
| nope-id `uuid()` (v4) | **~33.9M** | 🥇 самый быстрый pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~6.9M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~6.0M | |
| nope-id `uuidv7()` | ~12.9M | **~31x от v7 пакета `uuid`** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~58.2M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~6.2M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~27.4M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
...
| `secure-random-string` | ~366K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
| cuid2 `createId()` | ~5.4K | 24 символа, hash-производный | CSPRNG + SHA-3 |
...
|---------|-------------|
| `sortableId()` | ~13.5M оп/сек |
| `prefixedId()` | ~37M оп/сек |
| `uuid()` | ~33.6M оп/сек |
| `slugId()` | ~16.9M оп/сек |
| `shortId()` | ~21.1M оп/сек |
| `isValid()` | ~16.7M оп/сек |
| `uuidv7()` | ~12.9M оп/сек |
| `ulid()` | ~12.5M оп/сек |
| `monotonicFactory()` | ~15.3M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~14.6M оп/сек |
| `sqids.encode()` | ~846K оп/сек |
