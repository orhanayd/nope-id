...
|------|--------|---------|--------|
| Базовый (21 символ) | ~26.8M оп/сек | **~58.8M оп/сек** | **nope-id ~2.2x** |
| Короткий (10 символов) | ~35.4M оп/сек | **~53.2M оп/сек** | **nope-id ~1.5x** |
| Длинный (64 символа) | ~10.7M оп/сек | **~34.4M оп/сек** | **nope-id ~3.2x** |
| Свой алфавит | ~32M оп/сек | **~66.2M оп/сек** | **nope-id ~2.1x** |
| Пакетный (100 ID) | ~246K оп/сек | **~695K оп/сек** | **nope-id ~2.8x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~21.9M | C++ binding (только обычный v4) |
| nope-id `uuid()` (v4) | **~30.5M** | 🥇 самый быстрый pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~6.8M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~5.7M | |
| nope-id `uuidv7()` | ~14.7M | **~30x от v7 пакета `uuid`** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~58.8M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~6.2M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~26.8M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
| `sparkid` | ~10.7M | ~76 бит случайных (Base58, сортируемый по времени) | CSPRNG |
| `rndm` | ~2.8M | ~125 бит, но предсказуемые | `Math.random` (не безопасный) |
| `secure-random-string` | ~386K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
| cuid2 `createId()` | ~5.5K | 24 символа, hash-производный | CSPRNG + SHA-3 |
...
|---------|-------------|
| `sortableId()` | ~14.7M оп/сек |
| `prefixedId()` | ~36.4M оп/сек |
| `uuid()` | ~30.8M оп/сек |
| `slugId()` | ~16M оп/сек |
| `shortId()` | ~20.5M оп/сек |
| `isValid()` | ~15.4M оп/сек |
| `uuidv7()` | ~14.9M оп/сек |
| `ulid()` | ~13.7M оп/сек |
| `monotonicFactory()` | ~17.5M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~17.5M оп/сек |
| `sqids.encode()` | ~899K оп/сек |
