...
- **Daha Hızlı** - nanoid'den 1.5x ila 4x daha hızlı (CSPRNG, tam URL-safe alfabe); 5 temel benchmark'ın hepsini kazanıyor ([benchmark'lara bak](#performans))
<!-- bench:headline:end -->
...
<!-- bench:meta:start -->
_Son güncelleme: 2026-07-14, Node v22.x, linux/x64 (yerel)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Temel (21 karakter) | ~25.6M op/sn | **~57.7M op/sn** | **nope-id ~2.3x** |
| Küçük (10 karakter) | ~34.8M op/sn | **~54.7M op/sn** | **nope-id ~1.6x** |
| Büyük (64 karakter) | ~9.8M op/sn | **~36.3M op/sn** | **nope-id ~3.7x** |
| Özel Alfabe | ~29M op/sn | **~53.2M op/sn** | **nope-id ~1.8x** |
| Toplu (100 ID) | ~212K op/sn | **~747K op/sn** | **nope-id ~3.5x** |
...
| Üretici | op/sn | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~7.9M | C++ binding (yalnız düz v4) |
| nope-id `uuid()` (v4) | **~24.9M** | 🥇 en hızlı saf-JS v4 |
| `@lukeed/uuid` `v4()` | ~6.3M | optimize saf-JS v4 |
| `uuid` package `v4()` | ~5.8M | |
| nope-id `uuidv7()` | ~12M | **`uuid` paketinin v7'sinin ~22x'i** |
| `uuid` package `v7()` | ~555K | |
...
| nope-id `ulid()` | **~12.6M** |
| `ulid` package | ~31K |
| nope-id `monotonicFactory()` | **~15.2M** |
| `ulid` package (monotonic) | ~2.7M |
...
| Üretici | op/sn |
|---|---|
| nope-id `sortableId()` (22-char Crockford) | **~13.8M** |
| `sparkid` (21-char Base58) | ~10.3M |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~57.7M** | **~126 bit (64-karakter URL-safe)** | **CSPRNG** |
| `uid/secure` | ~5.8M | ~84 bit (16-karakter hex) | CSPRNG |
| nanoid | ~25.6M | ~126 bit (64-karakter URL-safe) | CSPRNG |
| `sparkid` | ~10.3M | ~76 bit rastgele (Base58, zaman-sıralı) | CSPRNG |
| `rndm` | ~2.7M | ~125 bit, ama öngörülebilir | `Math.random` (güvenli değil) |
| `secure-random-string` | ~407K | ~126 bit (base64, URL-safe değil) | CSPRNG |
| cuid2 `createId()` | ~5.3K | 24-karakter, hash-türevli | CSPRNG + SHA-3 |
...
| Özellik | Performans |
|---------|-------------|
| `sortableId()` | ~13.8M op/sn |
| `prefixedId()` | ~37.1M op/sn |
| `uuid()` | ~24.3M op/sn |
| `slugId()` | ~17.3M op/sn |
| `shortId()` | ~22.2M op/sn |
| `isValid()` | ~17.6M op/sn |
| `uuidv7()` | ~12.1M op/sn |
| `ulid()` | ~12.9M op/sn |
| `monotonicFactory()` | ~16.4M op/sn |
| `snowflake` (factory) | ~4.1M op/sn |
| `objectId()` | ~15.6M op/sn |
| `sqids.encode()` | ~805K op/sn |
