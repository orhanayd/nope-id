...
|------|--------|---------|--------|
| Temel (21 karakter) | ~34.5M op/sn | **~74.6M op/sn** | **nope-id ~2.2x** |
| Küçük (10 karakter) | ~44.8M op/sn | **~66.5M op/sn** | **nope-id ~1.5x** |
| Büyük (64 karakter) | ~13.8M op/sn | **~43.7M op/sn** | **nope-id ~3.2x** |
| Özel Alfabe | ~41.2M op/sn | **~83.3M op/sn** | **nope-id ~2x** |
| Toplu (100 ID) | ~319K op/sn | **~982K op/sn** | **nope-id ~3.1x** |
...
| Üretici | op/sn | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~25.1M | C++ binding (yalnız düz v4) |
| nope-id `uuid()` (v4) | **~37.7M** | 🥇 en hızlı saf-JS v4 |
| `@lukeed/uuid` `v4()` | ~8.3M | optimize saf-JS v4 |
| `uuid` package `v4()` | ~7.5M | |
| nope-id `uuidv7()` | ~10.7M | **`uuid` paketinin v7'sinin ~21x'i** |
...
| nope-id `ulid()` | **~14.9M** |
| `ulid` package | ~37K |
| nope-id `monotonicFactory()` | **~9.7M** |
| `ulid` package (monotonic) | ~2.8M |
...
| Üretici | op/sn |
|---|---|
| nope-id `sortableId()` (22-char Crockford) | **~16.7M** |
| `sparkid` (21-char Base58) | ~12M |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~74.6M** | **~126 bit (64-karakter URL-safe)** | **CSPRNG** |
| `uid/secure` | ~8.0M | ~84 bit (16-karakter hex) | CSPRNG |
| nanoid | ~34.5M | ~126 bit (64-karakter URL-safe) | CSPRNG |
| `sparkid` | ~12M | ~76 bit rastgele (Base58, zaman-sıralı) | CSPRNG |
| `rndm` | ~3.5M | ~125 bit, ama öngörülebilir | `Math.random` (güvenli değil) |
| `secure-random-string` | ~448K | ~126 bit (base64, URL-safe değil) | CSPRNG |
| cuid2 `createId()` | ~6.6K | 24-karakter, hash-türevli | CSPRNG + SHA-3 |
...
| Özellik | Performans |
|---------|-------------|
| `sortableId()` | ~16.7M op/sn |
| `prefixedId()` | ~47.4M op/sn |
| `uuid()` | ~38.8M op/sn |
| `slugId()` | ~9.4M op/sn |
| `shortId()` | ~20.8M op/sn |
| `isValid()` | ~17.8M op/sn |
| `uuidv7()` | ~10.7M op/sn |
| `ulid()` | ~14.9M op/sn |
| `monotonicFactory()` | ~9.6M op/sn |
| `snowflake` (factory) | ~4.1M op/sn |
| `objectId()` | ~16.3M op/sn |
| `sqids.encode()` | ~277K op/sn |
