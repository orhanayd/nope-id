...
- **Быстрее** - в 5–8 раз быстрее nanoid (CSPRNG, полный URL-безопасный алфавит); выигрывает все 5 основных бенчмарков ([см. бенчмарки](#производительность))
<!-- bench:headline:end -->
...
<!-- bench:meta:start -->
_Последнее обновление: 2026-05-24, Node v21.x, darwin/arm64 (локально)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~7.2M оп/сек | **~52.2M оп/сек** | **nope-id ~7.2x** |
| Короткий (10 символов) | ~13.6M оп/сек | **~63.4M оп/сек** | **nope-id ~4.7x** |
| Длинный (64 символа) | ~2.9M оп/сек | **~22.1M оп/сек** | **nope-id ~7.7x** |
| Свой алфавит | ~6.6M оп/сек | **~30.9M оп/сек** | **nope-id ~4.7x** |
| Пакетный (100 ID) | ~70K оп/сек | **~532K оп/сек** | **nope-id ~7.6x** |
...
nope-id - самая быстрая **JavaScript** библиотека для очень распространённой задачи: **криптографически безопасный URL-безопасный id на полном 64-символьном алфавите** (ниша nanoid). В этой категории он обгоняет nanoid в ~5–8 раз на стандартных размерах (4.7x на 10 символах, 7.2x на дефолтных 21, 7.7x на 64), а также производит UUIDv7 и ULID гораздо быстрее, чем специализированные пакеты (~**8x** к v7 пакета `uuid` и ~**60x** к пакету `ulid`).
...
- **Нативный `crypto.randomUUID()`:** встроенный C++, самый быстрый для обычного v4, не JS-библиотека. Если вам нужен только v4 UUID, используйте его.
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | **~26.2M** | 🥇 самый быстрый для обычного v4 |
| nope-id `uuid()` (v4) | ~24.8M | на уровне `@lukeed/uuid`, быстрее пакета `uuid` |
| `@lukeed/uuid` `v4()` | ~8.1M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~8.0M | |
| nope-id `uuidv7()` | ~7.4M | **~8x от v7 пакета `uuid`** |
| `uuid` package `v7()` | ~871K | |
<!-- bench:uuid-table:end -->

**Честный подход:** если всё, что вам нужно, - это случайный v4 UUID, **встроенный в Node `crypto.randomUUID()` далеко всех быстрее, используйте его.** nope-id не пытается обогнать native там. Его ценность в **широте**: UUIDv7, ULID, Snowflake, ObjectId, Sqids, типизированные ID и короткие ID в стиле nanoid (большинство из которых пакет `uuid` и native не предлагают), плюс быстрее nanoid для URL-безопасных ID и быстрее пакета `uuid` (особенно v7), всё это dual-module и zero-dependency.
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~52.2M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~9.8M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~7.2M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
| `rndm` | ~3.5M | ~125 бит, но предсказуемые | `Math.random` (не безопасный) |
| `secure-random-string` | ~661K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
| cuid2 `createId()` | ~7.1K | 24 символа, hash-производный | CSPRNG + SHA-3 |
...
- **nanoid** точно соответствует энтропии nope-id (тот же 64-символьный алфавит); nope-id просто в ~7.2 раза быстрее на дефолтном 21-символьном размере.

nope-id - единственный ряд, у которого есть **все три сразу**: максимальная энтропия на символ (126 бит), реальный CSPRNG и высочайшая скорость. Это и есть цель всего проекта - быть быстрым, никогда не тратя случайность ради этого. (Для обычного v4 UUID нативный `crypto.randomUUID()` всё ещё быстрее на 122 битах в C++, поэтому используйте его, когда вам нужен только UUID.)
...
|---------|-------------|
| `sortableId()` | ~8.9M оп/сек |
| `prefixedId()` | ~38.7M оп/сек |
| `uuid()` | ~24.7M оп/сек |
| `slugId()` | ~8.7M оп/сек |
| `shortId()` | ~19.7M оп/сек |
| `isValid()` | ~5.6M оп/сек |
| `uuidv7()` | ~7.4M оп/сек |
| `ulid()` | ~3.3M оп/сек |
| `monotonicFactory()` | ~14.4M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~10M оп/сек |
| `sqids.encode()` | ~327K оп/сек |
