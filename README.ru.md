...
<!-- bench:meta:start -->
_Последнее обновление: 2026-05-28, Node v22.x, darwin/arm64 (локально)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~7.3M оп/сек | **~51.2M оп/сек** | **nope-id ~7x** |
| Короткий (10 символов) | ~13.3M оп/сек | **~61.4M оп/сек** | **nope-id ~4.6x** |
| Длинный (64 символа) | ~2.9M оп/сек | **~23.5M оп/сек** | **nope-id ~8.2x** |
| Свой алфавит | ~7.9M оп/сек | **~31.4M оп/сек** | **nope-id ~4x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | **~27M** | 🥇 самый быстрый для обычного v4 |
| nope-id `uuid()` (v4) | ~22.6M | ~2.8x быстрее `@lukeed/uuid`, быстрее пакета `uuid` |
| `@lukeed/uuid` `v4()` | ~8.0M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~8.1M | |
| nope-id `uuidv7()` | ~8.6M | **~8x от v7 пакета `uuid`** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~51.2M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
...
| `secure-random-string` | ~805K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
...
|---------|-------------|
| `sortableId()` | ~10.4M оп/сек |
| `prefixedId()` | ~39.1M оп/сек |
| `uuid()` | ~22.7M оп/сек |
| `slugId()` | ~8.8M оп/сек |
| `shortId()` | ~19.8M оп/сек |
| `isValid()` | ~7.0M оп/сек |
| `uuidv7()` | ~8.7M оп/сек |
| `ulid()` | ~3.7M оп/сек |
| `monotonicFactory()` | ~16M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~9.6M оп/сек |
