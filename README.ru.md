...
- **Быстрее** - в 4–8 раз быстрее nanoid (CSPRNG, полный URL-безопасный алфавит); выигрывает все 5 основных бенчмарков ([см. бенчмарки](#производительность))
<!-- bench:headline:end -->
...
<!-- bench:meta:start -->
_Последнее обновление: 2026-05-25, Node v22.x, darwin/arm64 (локально)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~7.3M оп/сек | **~51.5M оп/сек** | **nope-id ~7.1x** |
| Короткий (10 символов) | ~13.8M оп/сек | **~62M оп/сек** | **nope-id ~4.5x** |
| Длинный (64 символа) | ~2.9M оп/сек | **~23.5M оп/сек** | **nope-id ~8.1x** |
| Свой алфавит | ~7.9M оп/сек | **~31.7M оп/сек** | **nope-id ~4x** |
| Пакетный (100 ID) | ~73K оп/сек | **~534K оп/сек** | **nope-id ~7.3x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | **~28M** | 🥇 самый быстрый для обычного v4 |
| nope-id `uuid()` (v4) | ~24.3M | ~2.9x быстрее `@lukeed/uuid`, быстрее пакета `uuid` |
| `@lukeed/uuid` `v4()` | ~8.3M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~8.1M | |
| nope-id `uuidv7()` | ~8.9M | **~9x от v7 пакета `uuid`** |
...
| nope-id `ulid()` | **~4.0M** |
| `ulid` package | ~60K |
| nope-id `monotonicFactory()` | **~17.4M** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~51.5M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~9.7M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~7.3M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
| `rndm` | ~3.6M | ~125 бит, но предсказуемые | `Math.random` (не безопасный) |
| `secure-random-string` | ~797K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
| cuid2 `createId()` | ~7.6K | 24 символа, hash-производный | CSPRNG + SHA-3 |
...
- **nanoid** точно соответствует энтропии nope-id (тот же 64-символьный алфавит); nope-id просто в <!-- bench:basic-21-ratio:start -->~7.1x<!-- bench:basic-21-ratio:end --> раза быстрее на дефолтном 21-символьном размере.
...
|---------|-------------|
| `sortableId()` | ~11.1M оп/сек |
| `prefixedId()` | ~39.7M оп/сек |
| `uuid()` | ~24.3M оп/сек |
| `slugId()` | ~8.8M оп/сек |
| `shortId()` | ~19.9M оп/сек |
| `isValid()` | ~7.6M оп/сек |
| `uuidv7()` | ~9.1M оп/сек |
| `ulid()` | ~4.2M оп/сек |
| `monotonicFactory()` | ~17.4M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~9.9M оп/сек |
| `sqids.encode()` | ~329K оп/сек |
