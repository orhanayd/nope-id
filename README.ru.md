...
<!-- bench:meta:start -->
_Последнее обновление: 2026-05-24, Node v26.x, ubuntu-latest (GitHub Actions)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~5.3M оп/сек | **~38.2M оп/сек** | **nope-id ~7.2x** |
| Короткий (10 символов) | ~10M оп/сек | **~41.2M оп/сек** | **nope-id ~4.1x** |
| Длинный (64 символа) | ~2.1M оп/сек | **~18.2M оп/сек** | **nope-id ~8.7x** |
| Свой алфавит | ~5.5M оп/сек | **~18.9M оп/сек** | **nope-id ~3.4x** |
| Пакетный (100 ID) | ~54K оп/сек | **~422K оп/сек** | **nope-id ~7.8x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | **~20M** | 🥇 самый быстрый для обычного v4 |
| nope-id `uuid()` (v4) | ~23.5M | на уровне `@lukeed/uuid`, быстрее пакета `uuid` |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~38.2M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~6.4M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~5.3M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
| `rndm` | ~2.7M | ~125 бит, но предсказуемые | `Math.random` (не безопасный) |
| `secure-random-string` | ~377K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
| cuid2 `createId()` | ~5.2K | 24 символа, hash-производный | CSPRNG + SHA-3 |
...
| `prefixedId()` | ~27.9M оп/сек |
| `uuid()` | ~23.6M оп/сек |
| `slugId()` | ~6.0M оп/сек |
| `shortId()` | ~13.1M оп/сек |
| `isValid()` | ~7.1M оп/сек |
| `uuidv7()` | ~5.3M оп/сек |
| `ulid()` | ~2.7M оп/сек |
