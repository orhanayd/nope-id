...
<!-- bench:meta:start -->
_Последнее обновление: 2026-07-13, Node v22.x, linux/x64 (локально)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~18.8M оп/сек | **~43.4M оп/сек** | **nope-id ~2.3x** |
| Короткий (10 символов) | ~27.2M оп/сек | **~42.3M оп/сек** | **nope-id ~1.6x** |
| Длинный (64 символа) | ~7.5M оп/сек | **~23.9M оп/сек** | **nope-id ~3.2x** |
| Свой алфавит | ~23.2M оп/сек | **~41.1M оп/сек** | **nope-id ~1.8x** |
| Пакетный (100 ID) | ~172K оп/сек | **~517K оп/сек** | **nope-id ~3x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~6.3M | C++ binding (только обычный v4) |
| nope-id `uuid()` (v4) | **~11.6M** | 🥇 самый быстрый pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~5.1M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~4.3M | |
| nope-id `uuidv7()` | ~7.4M | **~17x от v7 пакета `uuid`** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~43.4M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~5.1M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~18.8M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
| `sparkid` | ~9.1M | ~76 бит случайных (Base58, сортируемый по времени) | CSPRNG |
| `rndm` | ~2.2M | ~125 бит, но предсказуемые | `Math.random` (не безопасный) |
| `secure-random-string` | ~357K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
| cuid2 `createId()` | ~4.7K | 24 символа, hash-производный | CSPRNG + SHA-3 |
...
|---------|-------------|
| `sortableId()` | ~13.1M оп/сек |
| `prefixedId()` | ~27.4M оп/сек |
| `uuid()` | ~12.9M оп/сек |
| `slugId()` | ~6.3M оп/сек |
| `shortId()` | ~13.9M оп/сек |
| `isValid()` | ~14M оп/сек |
| `uuidv7()` | ~7.2M оп/сек |
| `ulid()` | ~11.3M оп/сек |
| `monotonicFactory()` | ~7.6M оп/сек |
| `snowflake` (factory) | ~4.0M оп/сек |
| `objectId()` | ~13.4M оп/сек |
| `sqids.encode()` | ~178K оп/сек |
