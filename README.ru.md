...
<!-- bench:meta:start -->
_Последнее обновление: 2026-05-27, Node v22.x, darwin/arm64 (локально)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~7.0M оп/сек | **~48.7M оп/сек** | **nope-id ~7x** |
| Короткий (10 символов) | ~12.9M оп/сек | **~55.7M оп/сек** | **nope-id ~4.3x** |
| Длинный (64 символа) | ~2.6M оп/сек | **~20.9M оп/сек** | **nope-id ~8x** |
| Свой алфавит | ~6.8M оп/сек | **~27.8M оп/сек** | **nope-id ~4.1x** |
| Пакетный (100 ID) | ~73K оп/сек | **~532K оп/сек** | **nope-id ~7.3x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~13.8M | C++ binding (только обычный v4) |
| nope-id `uuid()` (v4) | **~21.9M** | 🥇 самый быстрый pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~7.9M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~7.7M | |
| nope-id `uuidv7()` | ~8.2M | **~8x от v7 пакета `uuid`** |
| `uuid` package `v7()` | ~973K | |
...
| nope-id `ulid()` | **~3.3M** |
| `ulid` package | ~58K |
| nope-id `monotonicFactory()` | **~15.7M** |
...
| Генератор | оп/сек |
|---|---|
| nope-id `sortableId()` (22-char Crockford) | ~9.6M |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~48.7M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~9.6M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~7.0M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
...
| `secure-random-string` | ~775K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
...
|---------|-------------|
| `sortableId()` | ~9.6M оп/сек |
| `prefixedId()` | ~36.2M оп/сек |
| `uuid()` | ~21.6M оп/сек |
| `slugId()` | ~8.3M оп/сек |
| `shortId()` | ~19.2M оп/сек |
| `isValid()` | ~6.6M оп/сек |
| `uuidv7()` | ~8.1M оп/сек |
| `ulid()` | ~3.3M оп/сек |
| `monotonicFactory()` | ~15.3M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~8.9M оп/сек |
| `sqids.encode()` | ~317K оп/сек |
