...
- **Быстрее** - в 2–10 раз быстрее nanoid (CSPRNG, полный URL-безопасный алфавит); выигрывает все 5 основных бенчмарков ([см. бенчмарки](#производительность))
<!-- bench:headline:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~5.3M оп/сек | **~39.9M оп/сек** | **nope-id ~7.5x** |
| Короткий (10 символов) | ~10.2M оп/сек | **~44.9M оп/сек** | **nope-id ~4.4x** |
| Длинный (64 символа) | ~2.0M оп/сек | **~19.3M оп/сек** | **nope-id ~9.6x** |
| Свой алфавит | ~5.8M оп/сек | **~12.4M оп/сек** | **nope-id ~2.1x** |
| Пакетный (100 ID) | ~56K оп/сек | **~439K оп/сек** | **nope-id ~7.9x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~21.5M | C++ binding (только обычный v4) |
| nope-id `uuid()` (v4) | **~25.4M** | 🥇 самый быстрый pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~7.0M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~6.0M | |
| nope-id `uuidv7()` | ~5.2M | **~12x от v7 пакета `uuid`** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~39.9M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
...
| `secure-random-string` | ~408K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
| cuid2 `createId()` | ~5.7K | 24 символа, hash-производный | CSPRNG + SHA-3 |
...
| `uuidv7()` | ~5.0M оп/сек |
| `ulid()` | ~2.8M оп/сек |
| `monotonicFactory()` | ~8.3M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~7.2M оп/сек |
| `sqids.encode()` | ~211K оп/сек |
