...
<!-- bench:meta:start -->
_Последнее обновление: 2026-05-25, Node v26.x, ubuntu-latest (GitHub Actions)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~5.3M оп/сек | **~37.7M оп/сек** | **nope-id ~7.1x** |
| Короткий (10 символов) | ~10.1M оп/сек | **~42.9M оп/сек** | **nope-id ~4.2x** |
| Длинный (64 символа) | ~2.1M оп/сек | **~17.3M оп/сек** | **nope-id ~8.1x** |
| Свой алфавит | ~5.6M оп/сек | **~20.1M оп/сек** | **nope-id ~3.6x** |
| Пакетный (100 ID) | ~54K оп/сек | **~389K оп/сек** | **nope-id ~7.2x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~22.1M | C++ binding (только обычный v4) |
| nope-id `uuid()` (v4) | **~25.2M** | 🥇 самый быстрый pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~6.9M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~6.0M | |
| nope-id `uuidv7()` | ~5.5M | **~11x от v7 пакета `uuid`** |
...
| nope-id `ulid()` | **~3.3M** |
| `ulid` package | ~37K |
| nope-id `monotonicFactory()` | **~10.0M** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~37.7M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
...
| `secure-random-string` | ~390K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
...
| `uuidv7()` | ~5.4M оп/сек |
| `ulid()` | ~3.3M оп/сек |
| `monotonicFactory()` | ~9.7M оп/сек |
| `snowflake` (factory) | ~4.1M оп/сек |
| `objectId()` | ~7.0M оп/сек |
| `sqids.encode()` | ~208K оп/сек |
