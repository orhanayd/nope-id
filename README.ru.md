...
<!-- bench:meta:start -->
_Последнее обновление: 2026-05-28, Node v26.x, ubuntu-latest (GitHub Actions)._
<!-- bench:meta:end -->
...
|------|--------|---------|--------|
| Базовый (21 символ) | ~5.5M оп/сек | **~39.5M оп/сек** | **nope-id ~7.2x** |
| Короткий (10 символов) | ~9.9M оп/сек | **~44.2M оп/сек** | **nope-id ~4.5x** |
| Длинный (64 символа) | ~2.0M оп/сек | **~18.8M оп/сек** | **nope-id ~9.3x** |
| Свой алфавит | ~5.7M оп/сек | **~12.4M оп/сек** | **nope-id ~2.2x** |
| Пакетный (100 ID) | ~56K оп/сек | **~435K оп/сек** | **nope-id ~7.8x** |
...
| Генератор | оп/сек | |
|---|---|---|
| `crypto.randomUUID()` (Node native, v4) | ~21.2M | C++ binding (только обычный v4) |
| nope-id `uuid()` (v4) | **~25M** | 🥇 самый быстрый pure-JS v4 |
| `@lukeed/uuid` `v4()` | ~6.9M | оптимизированный pure-JS v4 |
| `uuid` package `v4()` | ~5.9M | |
| nope-id `uuidv7()` | ~5.3M | **~12x от v7 пакета `uuid`** |
...
|---|---|---|---|
| **nope-id `nopeid()`** | **~39.5M** | **~126 бит (64-символьный URL-безопасный)** | **CSPRNG** |
| `uid/secure` | ~6.3M | ~84 бит (16-символьный hex) | CSPRNG |
| nanoid | ~5.5M | ~126 бит (64-символьный URL-безопасный) | CSPRNG |
...
| `secure-random-string` | ~394K | ~126 бит (base64, не URL-безопасный) | CSPRNG |
...
- **nanoid** точно соответствует энтропии nope-id (тот же 64-символьный алфавит); nope-id просто в <!-- bench:basic-21-ratio:start -->~7.2x<!-- bench:basic-21-ratio:end --> раза быстрее на дефолтном 21-символьном размере.
...
| `prefixedId()` | ~28.4M оп/сек |
| `uuid()` | ~24.9M оп/сек |
| `slugId()` | ~5.6M оп/сек |
| `shortId()` | ~11.3M оп/сек |
| `isValid()` | ~6.7M оп/сек |
| `uuidv7()` | ~5.3M оп/сек |
| `ulid()` | ~2.8M оп/сек |
